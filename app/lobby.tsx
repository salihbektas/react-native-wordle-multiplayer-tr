  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(dbRef, snapshot => {
      if(snapshot.exists()){
        ...
        if(!(data.playerList as string[]).find(playerNameInList => playerName === playerNameInList)){
          off(dbRef)
          router.back()
        }
  ...
  function kickPlayer(playerName: string) {
    function kick(params:string) {
      runTransaction(child(dbRootRef, dbRefName), (serverState:ServerType) => {
        if(serverState){
          serverState.playerCount--
          serverState.playerList = serverState.playerList.filter(playerNameOnList => params !== playerNameOnList)
    ...
    if(Platform.OS === 'web' && confirm(`${playerName} oyuncusunu atmak istiyormusunuz ?`)){
      kick(playerName)
    }
    else{
      Alert.alert(`${playerName} oyuncusunu atmak istiyormusunuz ?`,'',
        [
          ...
          {text: 'Evet', onPress: () => { kick(playerName) }}
        ]
      )
    }
  }
    ...
    <View style={styles.main}>
      ...
      {playerList.map((playerNameInList, index) => 
        <View key={index} style={styles.listRow} >
          ...
          {amIHost && playerName !== playerNameInList && <Pressable onPress={() => {kickPlayer(playerNameInList)}}>
